...
const VehiclesPage: React.FC = () => {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (res.data.success && res.data.data.tenant?.settings) {
        const settings = res.data.data.tenant.settings;
        if (settings.hubs) setHubs(settings.hubs);
        if (settings.definitions?.currencies) {
            setCurrencies(settings.definitions.currencies);
        } else {
            setCurrencies([{code: 'TRY', symbol: '₺'}, {code: 'USD', symbol: '$'}, {code: 'EUR', symbol: '€'}]);
        }
      }
    } catch (error) {
              ...
              },
              {
                ...
                children: (
                  <>
                    <Row gutter={16}>
                      <Col span={12}>
                        <Form.Item label="Para Birimi" name="currency" rules={[{ required: true, message: 'Para birimi seçin' }]}>
                          <Select placeholder="Seçiniz">
                            {currencies.map(c => (
                    ...
                    <div style={{ marginTop: 24, marginBottom: 16 }}>
                        <Typography.Text strong style={{ fontSize: 16 }}>Bölge Bazlı Poligon Fiyatları</Typography.Text>
                        <Typography.Paragraph type="secondary" style={{ fontSize: 12, marginTop: 4 }}>
                            Aşağıdan istediğiniz bölgeleri seçip o bölgeye özel sabit fiyat ve bölge dışına çıkıldığında (aşım) hesaplanacak km fiyatı belirleyebilirsiniz.
                      ...
                      {(fields, { add, remove }) => (
                        <>
                          {fields.map(({ key, name, ...restField }) => (
                            <Row key={key} gutter={16} align="middle" style={{ marginBottom: 12, padding: 12, background: '#fafafa', borderRadius: 8, border: '1px solid #f0f0f0' }}>
                              <Col span={5}>
                                <Form.Item
                                  {...restField}
                                  name={[name, 'baseLocation']}
                                  rules={[{ required: true, message: 'Kalkış / Varış Noktası seçin' }]}
                                  style={{ marginBottom: 0 }}
                                >
                                  <Select placeholder="Base (Örn: AYT)" style={{ width: '100%' }}>
                                    {hubs.map((h: any) => (
                              ...
                              <Col span={6}>
                                <Form.Item
                                  ...
                                  style={{ marginBottom: 0 }}
                                >
                                  <Select placeholder="Bölge Seç" style={{ width: '100%' }}>
                                    {zones.map(z => (
                              ...
                              <Col span={5}>
                                <Form.Item
                                  ...
                                  style={{ marginBottom: 0 }}
                                >
                                  <InputNumber min={0} step={1} precision={2} placeholder="Sabit Fiyat" style={{ width: '100%' }} />
                                </Form.Item>
                              </Col>
                              <Col span={6}>
                                <Form.Item
                                  ...
                                  style={{ marginBottom: 0 }}
                                >
                                  <InputNumber min={0} step={0.1} precision={2} placeholder="Aşım Km Fiyat" style={{ width: '100%' }} />
                                </Form.Item>
                              </Col>
