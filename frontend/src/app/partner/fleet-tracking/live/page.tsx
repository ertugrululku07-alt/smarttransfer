...
import { Alert, Badge, Button, Card, Empty, Form, Input, Modal, Select, Space, Spin, Table, Tag, Tooltip, message } from 'antd';
import {
    ...
    <div style={{ display: 'grid', gap: 14 }}>
        ...
        extra={
          <Space>
            <Tag color={autoRefresh ? 'green' : 'default'}>
              <Badge dot status={autoRefresh ? 'success' : 'default'} /> Otomatik yenileme · {autoRefresh ? '15 sn' : 'KAPALI'}
            </Tag>
      ...
      >
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 10, marginBottom: 14 }}>
          ...
          <div className="ps-kpi" style={{ borderLeft: '3px solid #94a3b8' }}><div className="ps-kpi__label">Çevrimdışı</div><div className="ps-kpi__value">{vehicles.length - onlineCount}</div></div>
          <div className="ps-kpi" style={{ borderLeft: '3px solid #6366f1' }}><div className="ps-kpi__label">Cihaz Sayısı</div><div className="ps-kpi__value">{devices.length}</div></div>
        </div>
        ...
        <Table
          ...
          columns={[
            ...
            { title: 'Konum', render: (_: any, r: any) => {
              ...
              return <a href={url} target="_blank" rel="noreferrer"><EnvironmentOutlined /> {r.lastTelemetry.lat.toFixed(5)}, {r.lastTelemetry.lng.toFixed(5)}</a>;
            } },
      ...
      <Card size="small" title={<span><ApiOutlined style={{ marginRight: 8, color: '#6366f1' }} /> IoT Cihazları & API Anahtarları</span>}
        extra={
          <Button type="primary" icon={<PlusOutlined />} onClick={() => { form.resetFields(); setCreateModal(true); }}>Yeni Cihaz</Button>
        }
      >
        <Alert
          showIcon
          ...
          message="Telemetri ingestion endpoint"
          description={
            <div style={{ fontSize: 12 }}>
              GPS cihazınız HTTP POST ile verileri şu adrese göndermeli:{' '}
              <code style={{ background: '#fff', padding: '2px 6px', borderRadius: 4 }}>POST /api/partner-fleet/telemetry/ingest</code>{' '}
              · Header: <code>X-API-Key: &lt;cihaz key&gt;</code> · Body: <code>{`{ vehicleId?, timestamp, lat, lng, speed, odometer, fuelLevel, engineStatus }`}</code> veya <code>{`{ events: [...] }`}</code>
            </div>
          }
        />
      ...
      <Modal title="Yeni IoT Cihazı" open={createModal} onCancel={() => setCreateModal(false)} onOk={onCreateDevice} okText="Oluştur" cancelText="Vazgeç">
        <Form form={form} layout="vertical">
          ...
          <Form.Item label="Bağlı Araç (opsiyonel)" name="vehicleId" help="Bağlı araç seçilirse cihaz yalnız bu aracın verisini yazabilir">
      ...
      <Modal title="🔑 Cihaz API Anahtarı" open={keyModal.open} onCancel={() => setKeyModal({ open: false })} footer={null}>
        <Alert showIcon type="warning" style={{ marginBottom: 12 }} message="Bu anahtar SADECE BİR KEZ gösterilir!" description="Kopyalayın ve GPS cihazınıza kaydedin. Sayfayı kapattığınızda tekrar görüntüleyemezsiniz." />
