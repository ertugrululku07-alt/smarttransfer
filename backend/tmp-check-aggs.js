...
async function run() {
    ...
    vehicles.forEach(v => {
        ...
        (tr.fuel || []).forEach(f => sums.vehicles.out += Number(f.totalCost || 0));
        (tr.maintenance || []).forEach(m => sums.vehicles.out += Number(m.cost || 0));
        (tr.insurance || []).forEach(i => sums.vehicles.out += Number(i.cost || 0));
        (tr.inspection || []).forEach(i => sums.vehicles.out += Number(i.cost || 0));
    });
